...
def _write_to_db(collection_name, staff_name, adjusted_datetime, loadingsplash, app, db, is_clock_in):
    dt_object = dt.now()
    if adjusted_datetime is not None:
        dt_object = dt.strptime(adjusted_datetime, "%Y%m%d_%H%M")

    if PromptPopup(f'Clock {"in" if is_clock_in else "out"} {staff_name} at {(dt.strftime(dt_object, "%I:%M %p, %b %d, %Y"))}?').get():
...
def _validate_attendance_entry(collection_name, staff_name, adjusted_datetime, loadingsplash, app, db, is_clock_in):
    retrieved_entries = list(collection_name.find({"staff_name": staff_name}).sort({"date":-1, "time":-1}).limit(1))

    # admin override of attendance entry without validation
    if adjusted_datetime is not None:
        ic(adjusted_datetime, is_clock_in)
        _write_to_db(collection_name, staff_name, adjusted_datetime, loadingsplash, app, db, is_clock_in)
        return
...
def callback(app=None, adjusted_datetime=None, adjusted_staffname=None, is_clock_in=True):
    if app is None:
    ...
    def start_validate_attendance():
        threading.Thread(
            target=_validate_attendance_entry,
            args=(collection_name, staff_name, adjusted_datetime, loadingsplash, app, db, is_clock_in),
            daemon=True
        ).start()

    loadingsplash.show(task=start_validate_attendance)
